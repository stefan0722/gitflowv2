...
releaseBranchName = input("Please enter name of release branch: release-")
releaseVersion = input("Please enter the version of the release: ")
alreadyExists = subprocess.call(["git","checkout","-b","release-" + releaseBranchName,"develop"])
if alreadyExists is 128 :
    subprocess.call(["git","checkout","feature-" + releaseVersion],shell=True)

print("\n\n-- Step 5:     Increasing version of release-" + releaseBranchName + " to " + releaseVersion)
...
subprocess.call(["git","commit","-m","Change release branch version to " + releaseVersion + "-SNAPSHOT",projectDir + "**pom.xml"])
...
if pushRequest is "Y":
    ...
    subprocess.call(["git","push",remoteUrl,"release-" + releaseBranchName])
